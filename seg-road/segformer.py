...
class SegFormer_Segmentation(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"        : "logs/ep060-loss0.061-val_loss0.092.pth",
        #----------------------------------------#
        #   所需要区分的类的个数+1
        #----------------------------------------#
        "num_classes"       : 2,
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        with torch.no_grad():
            ...
            #   图片传入网络进行预测
            #---------------------------------------------------#
            pr, out_connect, out_connect_d1 = self.net(images)
            #---------------------------------------------------#
            ...
            pr = pred_full + pred_connect + pred_connect_d1
            pr[pr > 0] = 1
    ...
    def get_miou_png(self, image):
        ...
        #     # image2 = image[:, :, :, ::-1]
        #     # image3 = image[:, :, ::-1, ::-1]
        #     # images = torch.concat((image, image1, image2, image3), axis=0)
        #
        #
        #     #---------------------------------------------------#
        #     #   图片传入网络进行预测
        ...
        with torch.no_grad():
            ...
            # ---------------------------------------------------#
            '''
            out_connect = nn.Sigmoid()(out_connect)
            out_connect_d1 = nn.Sigmoid()(out_connect_d1)
            out_connect_full = []
            out_connect = out_connect.data.cpu().numpy()
            out_connect_full.append(out_connect[0, ...])
            out_connect_full.append(out_connect[1, :, ::-1, :])
            ...
            pred_connect_d1[pred_connect_d1 >= 2] = 1
            '''
